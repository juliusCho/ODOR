...
export default {
  ...
  methods: {
    ...
    goTo(page) {
      if (page === '') {
        this.$emit('goTo', 'Home', this.forumKey, this.categoryId);
        return;
      }
      this.$emit('goTo', page, this.forumKey, this.categoryId);
    },
    async checkSession(page) {
      ...
      .then((res) => {
        this.$emit('goTo', res.data ? page : this.incl, this.forumKey, this.categoryId);
      });
    },
    loggedIn(boo) {
